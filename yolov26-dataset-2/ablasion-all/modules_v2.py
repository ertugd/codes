...
#
# Host adaylari, `parse_model` icindeki frozenset uyeliklerine gore secildi:
#   * non-repeat host: base_modules ICINDE, repeat_modules DISINDA
#     -> modul (c1, c2, *args) ile kurulur
#   * repeat host    : base_modules VE repeat_modules icinde
#     -> modul (c1, c2, n, *args) ile kurulur
#
# DIKKAT: ilk surumde non-repeat host olarak `FEM` kullaniliyordu. FEM
# upstream ultralytics'te YOK - bu kurulumdaki elle eklenmis
# `nn/modules/inn_modules_v2.py` ile geliyor. `pip install ultralytics==...`
# yapildigi anda o yama silinir ve FEM ismi kaybolur. Bu yuzden host
# adaylari artik UPSTREAM'de garanti olan isimlerden basliyor ve calisma
# aninda mevcut olan ilk aday seciliyor.
_HOST_CANDIDATES = {
    "CAA": ["Focus", "SPP", "GhostBottleneck", "FEM"],       # non-repeat
    "C3k2CAA": ["C3Ghost", "C3x", "RepC3", "C3TR"],          # repeat
}
...
def _pick_hosts(tasks):
    """Her custom modul icin, tasks namespace'inde gercekten var olan ilk
    host ismini secer. Hicbir aday bulunamazsa hata verir (sessizce yanlis
    kanal hesabiyla egitmektense patlamasi iyi)."""
    alias = {}
    for name, cands in _HOST_CANDIDATES.items():
        for c in cands:
            if hasattr(tasks, c):
                alias[name] = c
                break
            ...
            raise RuntimeError(
                f"{name} icin host modul bulunamadi. Denenen: {cands}. "
                "ultralytics surumu beklenenden cok farkli olabilir."
            )
    return alias
...
def register_v2(bifpn: bool = True, verbose: bool = True):
    ...
    Args:
        bifpn: True ise YAML'daki tum `Concat`'lar WeightedConcatN ile
            ...
            kalir - ablasyon icin.
    """
    global _PARSE_PATCHED, _ALIAS
    import ultralytics.nn.tasks as tasks
    import ultralytics.nn.modules as modules

    # --- host isimleri sec ve siniflarimizi bagla ---
    _ALIAS = _pick_hosts(tasks)
    setattr(tasks, _ALIAS["CAA"], CAA)
    setattr(tasks, _ALIAS["C3k2CAA"], C3k2CAA)
    ...
    if verbose:
        ...
        print(f"[modules_v2] kayit tamam (ultralytics {ultralytics.__version__}):")
        print(f"   CAA      -> Context Anchor Attention  [host: {_ALIAS['CAA']}]")
        print(f"   C3k2CAA  -> C3k2 + CAA                [host: {_ALIAS['C3k2CAA']}]")
        if bifpn:
