...
class RepConv(nn.Module):
    """
    Basit RepConv bloğu:
    Train sırasında 3x3 + 1x1 + identity branch kullanır.
    Bu iskelette deploy/fuse fonksiyonu eklemedik, önce eğitim etkisini test ediyoruz.
    """
...
class RepC3k2(nn.Module):
    """
    YOLO26n için shape-preserving RepC3k2.

    Giriş/çıkış kanal sayısı aynıdır.
    Bu yüzden C3k2 bloğundan sonra eklemek güvenlidir.

    YAML örnekleri:
      P3 için: - [-1, 1, RepC3k2, [64, 1, True, 0.5]]
      P4 için: - [-1, 1, RepC3k2, [128, 1, True, 0.5]]
      P5 için: - [-1, 1, RepC3k2, [256, 1, True, 0.5]]
    """
...
def register_repc3k2_for_ultralytics():
    """
    Ultralytics YAML parser'ın RepC3k2 sınıfını görebilmesi için.
    Bunu YOLO("custom.yaml") satırından önce çağır.
    """
